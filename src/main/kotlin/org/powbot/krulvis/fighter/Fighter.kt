...
import org.powbot.krulvis.api.extensions.teleports.poh.KOUREND_HOUSE_PORTAL
    ...
    [
        ...
        ScriptConfiguration(
            ...
            allowedValues = [USE_WEB_OPTION, DISABLE_TELEPORTS_OPTION, EDGEVILLE_GLORY, EDGEVILLE_MOUNTED_GLORY, FEROX_ENCLAVE_ROD, FEROX_ENCLAVE_JEWELLERY_BOX,
                CASTLE_WARS_ROD, CASTLE_WARS_JEWELLERY_BOX, LUNAR_ISLE_HOUSE_PORTAL, KOUREND_HOUSE_PORTAL, POISON_WASTE_SPIRIT_TREE_POH,
...
//</editor-fold>
class Fighter : KillerScript(true), UniqueLootTracker, RunePouchScript {
    ...
    fun onInventory(inventory: Map<Int, Int>) {
        ...
        //RunePouch
        if (ids.any { RunePouch.ids.contains(it) }) {
    ...
    val requiredInventory by lazy {
        inventoryOptions.filter { it.key > 0 }.map { InventoryRequirement(it.key, it.value) }
    }
    ...
    fun getEquipment(optionKey: String): List<EquipmentRequirement> {
        return EquipmentRequirement.forOption(getOption(optionKey))
    }
    ...
    override fun canBreak(): Boolean {
        return false
//        return lastLeaf.name == "WithdrawItems"
    }
    ...
    fun nearbyMonsters(): List<Npc> =
        Npcs.stream().action("Attack").within(centerTile, killRadius.toDouble()).filtered {
