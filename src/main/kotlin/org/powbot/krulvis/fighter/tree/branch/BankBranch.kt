...
class ShouldBank(script: Fighter) : Branch<Fighter>(script, "Should Bank") {
    ...
    override fun validate(): Boolean {
        if (script.forcedBanking) {
            script.logger.info("Forced banking!")
            return true
        }

        val ammo = script.ammo
        if (ammo != null && !ammo.item.hasWith()) {
            script.logger.info("Ammo not with us, banking")
            return true
        }

        val hasFood = Food.hasFood()
        val needsFood = Food.needsFood()
        val bankOpened = Bank.opened()
        val inventoryFull = Inventory.isFull()
        val prayerWith = Potion.PRAYER.hasWith()

        return !hasFood && (needsFood || bankOpened || (inventoryFull && !prayerWith))
    }
}
...
class ShouldWithdrawItems(script: Fighter) : Branch<Fighter>(script, "ShouldWithdrawItems?") {
    override val failedComponent: TreeComponent<Fighter> = SimpleLeaf(script, "Empty Containers") {
        if (Container.emptyAll()) {
            ...
            script.logger.info("Containers not empty")
        }
    }
