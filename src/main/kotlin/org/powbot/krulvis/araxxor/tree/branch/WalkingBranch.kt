...
class InLair(script: Araxxor) : Branch<Araxxor>(script, "InLair") {
    ...
    override val successComponent: TreeComponent<Araxxor> =
        ShouldConsume(script, ShouldPray(script), eatWhenPossible = true)
...
class InSpiderDungeon(script: Araxxor) : Branch<Araxxor>(script, "InSpiderDungeon") {
    ...
    val shouldConsume = ShouldConsume(script, SimpleLeaf(script, "Empty") {}, eatWhenPossible = false)
    ...
    override val successComponent: TreeComponent<Araxxor> = SimpleLeaf(script, "EnterLair") {
        ...
        if (web.distance() > 7) {
            ...
            if (shouldConsume.validate()) {
                shouldConsume.execute()
            }
        ...
        if (wall.distance() < 10) {
            ...
            Data.darkmeyerPath.traverse {
                Prayer.prayer(
                    ...
                    Npcs.stream().nameContains("Vyrewatch").any { it.distance() < 8 }
                )
            }
        }
    }
