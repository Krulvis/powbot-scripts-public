...
private val logger = LoggerFactory.getLogger("TeleportEquipment")
...
enum class TeleportEquipment(
    ...
    fun hasInfiniteCharges(): Boolean {
        return ids.size <= 2
    }
     ...
     */
    private fun Item.getCharges(): Int {
        if (hasInfiniteCharges()) return Int.MAX_VALUE
    ...
    override fun withdrawExact(amount: Int, worse: Boolean, wait: Boolean): Boolean {
        ...
        if (charges >= amount) {
            val invCount = getInventoryCount(false)
            if (hasInfiniteCharges() && invCount > 1) {
                Bank.deposit(getInventoryId(), invCount - 1)
            }
            return true
        }
    ...
    private fun getBankItemForCharges(minCharges: Int): Item? {
