...
class ShouldConsume<S : KrulScript>(
    ...
    private val waitForConsume: Boolean = true
    ...
    override val successComponent: TreeComponent<S> = SimpleLeaf(script, "Consuming") {
        ...
        val food = if (canSipPray && food == null) foodNoKaram() else food
        val pot = potion
        if (food != null) {
    ...
    private fun foodNoKaram(): Food? {
        val missingHp = missingHP()
        return foods.filterNot { it == Food.KARAMBWAN }.firstOrNull { missingHp > it.healing && it.hasWith() }
    }
    ...
    override fun validate(): Boolean {
        if (!script.canConsume()) return false
        food = food()
        potion = potion()
    ...
    companion object {
        var foods: Array<Food> = Food.entries.toTypedArray()
        var potions: Array<Potion> = Potion.entries.toTypedArray()
        var consumeTick = -3
