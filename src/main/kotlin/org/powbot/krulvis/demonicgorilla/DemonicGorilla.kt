...
@ScriptConfiguration.List(
    [
        ...
        ScriptConfiguration(
            SPECIAL_WEAPON_OPTION,
            "Special Attack Weapon?",
            optionType = OptionType.STRING,
            defaultValue = EMBERLIGHT,
            allowedValues = ["NONE", DDS, ARCLIGHT, EMBERLIGHT]
        ),
        ScriptConfiguration(
            RESURRECT_OPTION,
            "Resurrect spell?",
...
//</editor-fold>
class DemonicGorilla : KillerScript(true) {
    ...
    override fun onStart() {
        ...
        EventFlows.collectInventoryChanges { onInventoryChange(it) }
        EventFlows.collectNpcAnimationChanges { onAnimationChangeEvent(it) }
        EventFlows.collectTicks { onTickEvent(it) }
    }
    ...
    val specialWeapon by lazy { Weapon.entries.firstOrNull { it.name == getOption(SPECIAL_WEAPON_OPTION) } }
    ...
    val resurrectSpell by lazy { ResurrectSpell.entries.firstOrNull { it.name == getOption(RESURRECT_OPTION) } }
