...
class ProjectileScript : AbstractScript() {
    ...
    override fun onStart() {
        EventFlows.collectProjectileDestinationChanges { onProjectile(it) }
...
//		logger.info("Projectiles=${projectiles.count()}, onMe=${projectiles.count { it.target() == me }}, onMyTile=${projectiles.count { it.destination() == myTile }}")
    ...
    override fun poll() {
        sleep(600)
        val projs = projectiles
		if (projs.isNotEmpty()) {
			logger.info("There is dangerous projectile!")
            projs.forEach { (p, t) -> logger.info("Proj(destination=${p.destination()}, tile=${p.tile()}, distance=${p.distance()}, valid=${p.valid()})") }
		}
    }
