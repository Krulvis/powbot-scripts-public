...
object TelekineticRoom : MTARoom {
    ...
    private fun build() {
        moves = buildEdges(guardianDestination)
    }

    private fun guardianDestination() {
        guardianDestination = if (mazeGuardian.id == MAZE_GUARDIAN_MOVING) {
            val direction = Direction.fromGuardian()
            neighbour(mazeGuardian.tile(), direction)
        } else {
            mazeGuardian.tile()
        }
    }

    private fun buildEdges(start: Tile): Stack<Pair<Tile, Direction>> {
        ...
        while (!visit.isEmpty() && visitedN < 10000) {
            val next = visit.poll()
            closed.add(next)
            val neighbours = neighbours(next)
    ...
    private fun neighbours(tile: Tile): Array<Tile> {
        return arrayOf(
            neighbour(tile, Direction.NORTH),
            neighbour(tile, Direction.EAST),
            neighbour(tile, Direction.SOUTH),
            neighbour(tile, Direction.WEST)
        )
    }

    private fun neighbour(tile: Tile, direction: Direction): Tile {
        val dx: Int
        val dy: Int
        ...
        var travelTile = tile
        while (!travelTile.blocked(direction.blockFlag) && bounds.contains(travelTile)) {
